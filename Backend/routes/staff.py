...
def save_leave_document(file_storage):
    ...
    public_id = f"{waktu_wib().strftime('%Y%m%d%H%M%S')}-{uuid4().hex[:10]}"

    if extension in {"jpg", "jpeg", "png", "webp"}:
        upload_options = {
            "folder": "argotelo/leave_documents",
            "public_id": public_id,
            "resource_type": "image",
            "format": extension,
        }
    else:
        # Cloudinary raw-file public IDs retain the original extension.
        upload_options = {
            "folder": "argotelo/leave_documents",
    ...
    try:
        ...
        raise ValueError(f"Upload surat izin gagal: {error}") from error

    secure_url = upload.get("secure_url")
    if not secure_url:
        raise ValueError("Upload surat izin gagal: URL Cloudinary tidak tersedia")
